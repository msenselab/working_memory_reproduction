...
mystancode <- '
// linear encoding modelfunctions {
matrix predictor_rng(real[] x, int[] size, real ks, real ls, real kr, real sig_s, real sigma_pr2,  real mu_prior, real sig_mn, real[] par) {
...
model {
  ...
  for (i in 1:n)
  {
    ...
    y[i] ~ normal(mu_r[i], sqrt(sig_mn*sig_mn + sig_wm2[i])); 
  }
}
...
generated quantities {
  ...
  vector[n] log_lik;
  real log_lik_sum;
  predRP = predictor_rng(x, wmsize, ks, ls, kr, sig_s, sig_pr2, mu_pr, sig_mn, par);
  ynew = predictor_rng(xnew, wmsize_new, ks, ls, kr, sig_s, sig_pr2, mu_pr, sig_mn, par);
