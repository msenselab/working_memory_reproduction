...
%     xlswrite('Exp1_WMatProduction.xlsx', dat,1,'A2:P5761');
   ...
   allArray.Crr = allArray.TPresent == allArray.WMRP;
    ...
    %%plot but do not separate target present and absent
       numSubFig = ceil(sqrt(length(subfiles)));
%     figure();
%     for isub = 1:length(subfiles)
%         subplot(numSubFig,numSubFig,isub); hold on;
%         idx = mTabel.NSub == isub & mTabel.WMSize == 1 ;
%         plot(mTabel.curDur(idx), mTabel.mean_repDur(idx),'r.--');
%         idx = mTabel.NSub == isub & mTabel.WMSize == 3 ;
%         plot(mTabel.curDur(idx),mTabel.mean_repDur(idx),'g.--');      
%         idx = mTabel.NSub == isub & mTabel.WMSize == 5 ;
%         plot(mTabel.curDur(idx),mTabel.mean_repDur(idx),'b.--');     
% 
%              
