  ...
  try
    ...
    clc;
      startIntro = ['Trial Sequence: \n\n'  ...
          '1st - Time production:   feel the time duration            \n' ...
          '2nd - Time reproduction:  press and hold the DOWN arrow key   \n' ...
          '3rd - Memory presentation:  remember the squares  \n\n' ...
          '4th  - Memory test:  LEFT = yes       RIGHT = no ' ...
          ];
    ...
    %hour
    nTrlsBlk = 20;
    exp = CExp(2,[ 1 5 2],'blockRepetition', 6, 'blockFactors', 3); 
    %adjust the column of exp.seq
    tmp = exp.seq(:,4);
    exp.seq(:, 2:4) = exp.seq(:, 1:3);
    exp.seq(:,1) = (tmp -1)*2 + 1; % convert to 1,3,5
...
%     WaitSecs(1);
    ...
    for trialIndex = 1:exp.maxTrls
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%%
     %%1. stat time production-reproduction task
     %%%%%%%%%%%%%%%%%%%%%%%%%%
     % production
...
%         WaitSecs(0.25); % wait 250 ms
       ...
       %ITI = 1seconds
        ...
        %%%%%%%%%%%%%%%%%%%%
         % working memory stimuli presentation
         % 3. prepare stimuli at given condition(s)
       ...
       WaitSecs(1);
     %%%%%%%%%%%%%%%%%%%%    
      %4 choose a circle to test, WM Test Display
     % pick item to test
...
%   Screen('Preference', 'SkipSyncTests', 1);
   V  = CDisplay('bgColor',para.bkColor,'fontSize',para.fntSize,'monitorSize',para.monitor,...
        'viewDistance',para.viewDistance,'fullWindow',1);
