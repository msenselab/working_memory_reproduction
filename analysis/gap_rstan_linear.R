###########################################################################
# Hierarchical Bayesian modeling (linear encoding version)
# for manuscript: Duration reproduction under memory pressure:
...
funFitStan <- function(subdat, myrstanModel, alluseOneModel,  onemodelname){
  ...
  myinits <- list()
  parameters <- c("sig_s2","ks", "ls","ts", "kr", "mu_pr", "sig_mn2", "sig_pr2", "predRP", "ynew", "log_lik", "log_lik_sum") 
  init_mem1 <-  list(sig_s2=1, ks= 0.1, ls= 0.1, ts = 0.1, kr= 0.1, mu_pr=1, sig_pr2 = 0.5, sig_mn2 = 0.1)
