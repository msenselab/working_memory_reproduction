...
AllExpData <- dat %>% filter(Exp=="Exp5") # %>% filter(repDur>0.1 & repDur<2.5)

AllExpData$log_RP <- log(AllExpData$repDur)
AllExpData$log_dur <- log(AllExpData$curDur)
AllExpData$Gap <- AllExpData$Gap * 1000

# AllExpData$RP_bias = AllExpData$repDur - AllExpData$curDur
# AllExpData = AllExpData %>% filter(abs(RP_bias) < 1) %>% filter(repDur > 0.2)
AllExpData
```
...
runModelonlocal<- function(AllExpData, myrstanModel){
  ...
  for (expName in expList) {
    ...
    # subList <- unique(data_exp$NSub)[0:1]
    sub_exp <-  split(data_exp, data_exp$NSub) # split data for each subject
    for (subNo in subList) {
      print(paste0('Now run rstan model ', expName,' on Subject No.',subNo, ' in ', expName))
...
funMergeMRlt <- function(explist, path, modelversion){
  ...
  write.csv(Bayparlist, paste0(getwd(), "/", path, "/rlt/AllDat_Bayparlist_", modelversion, "_2025.csv"))
  write.csv(AllDat_predY, paste0( getwd(), "/",path, "/rlt/AllDat_predY_", modelversion, "_2025.csv"))
  write.csv(AllDat_newY, paste0(getwd(), "/",path, "/rlt/AllDat_newY_", modelversion, "_2025.csv"))
