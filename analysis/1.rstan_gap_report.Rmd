...
```{r}
library(BayesFactor)
mpredY_sub$NSub = as.numeric(mpredY_sub$NSub)

bfFull = lmBF(RP_Bias ~ WMSize + curDur + WMSize:curDur + NSub, data = mpredY_sub%>%filter(Exp =='Exp. 1'), whichRandom = "NSub")

## Bayes factor of main effects only against null
bfMain = lmBF(RP_Bias ~ WMSize + curDur + NSub, data = mpredY_sub%>%filter(Exp =='Exp. 1'), whichRandom = "NSub")
## Compare the main-effects only model to the full model
bfMain / bfFull


## Bayes factor of main effects only against null
bfWMSize = lmBF(RP_Bias ~ WMSize + NSub, data = mpredY_sub%>%filter(Exp =='Exp. 1'), whichRandom = "NSub")
## Compare the main-effects only model to the full model
bfWMSize / bfFull


## Bayes factor of main effects only against null
bfDur = lmBF(RP_Bias ~ curDur + NSub, data = mpredY_sub%>%filter(Exp =='Exp. 1'), whichRandom = "NSub")
## Compare the main-effects only model to the full model
bfDur / bfFull
```
