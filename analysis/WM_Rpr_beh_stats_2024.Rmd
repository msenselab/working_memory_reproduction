...
# post hoc tests
pairwise.t.test(x = wmCrsExp$m_WMCrr, g = wmCrsExp$ExpName, paired = TRUE, p.adjust.method = 'holm')
...
av1 = mRep %>% filter(ExpName=='Encoding2' | ExpName=='Reproduction2'| ExpName=='baseline2') %>% 
  ...
  ezANOVA(dv = m_bias, wid = NSub, within = .(DurLevel, Load, ExpName), detailed = TRUE) %>%
  calParEta2()
print(av1)
print(GGcr(av1))
...
mRep_cti %>% filter(ExpName=='Encoding2' | ExpName=='Reproduction2'| ExpName=='baseline2') %>% 
  group_by(NSub,ExpName) %>% summarise(m_cti = mean(cti), n = n()) -> mRep_cti_6
pairwise.t.test(x = mRep_cti_6$m_cti, g = mRep_cti_6$ExpName, paired = TRUE, p.adjust.method = 'holm')
```

Post hoc pariwise t-test on general bias
