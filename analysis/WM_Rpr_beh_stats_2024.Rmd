...
tmp = subset(mBias %>% filter(ExpName=='Encoding2' | ExpName=='Reproduction2'| ExpName=='baseline2'), 
             select=c(ExpName, Load, gap, NSub, ms_bias)) %>% 
    pivot_wider(names_from = c("ExpName","Load","gap"), values_from = c(ms_bias), names_sep="_")
write.csv(tmp, paste0(getwd(),"/bhvrlt/DataAna_20240914/gBias_Exp6abc.csv"))
...
pairwise.t.test(x = mRep_mBias_6$mBias, g = mRep_mBias_6$Load, paired = TRUE, p.adjust.method = 'holm')
...
# Post hoc pairwise t-test on general bias by Loads
...
av_mB6abc = mBias_6abc %>% 
  ezANOVA(dv = ms_bias, wid = NSub, within = .(Load,ExpName), detailed = TRUE) %>%
  calParEta2()
print(av_mB6abc)
print(GGcr(av_mB6abc))
