...
def hModel(dat, constrain):
    ...
    with pm.Model() as WMmodel:
        ...
        if constrain[0] == 1:
            ...
            k_s =  pm.Deterministic('k_s', k_s0 * epsilon_k) 
        else:
            k_s = np.zeros(nsub)
        ...
        if constrain[1] == 1:
            ...
            l_s = pm.Deterministic('l_s', l_s0 * epsilon_k)
        else:
            l_s = np.zeros(nsub)
        ...
        if constrain[3] == 1:
            ...
            k_r =  pm.Deterministic('k_r', k_r0 * epsilon_k)
        else:
            k_r = np.zeros(nsub) 
...
### Experimentwise model parameters
...
### Encoding only and Reproduction only parameters
