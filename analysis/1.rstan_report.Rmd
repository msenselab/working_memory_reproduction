...
write_csv(mpredY_sub_new%>%filter(Exp == 'Exp4'), paste0(modelPath, '/rlt/RP_Bias_exp4.csv'))
...
write_csv(mpredY_sub_WMsize%>%filter(Exp == 'Exp4'), paste0(modelPath, '/rlt/RP_Bias_WMsize_exp4.csv'))
...
RP_bias  <- ggplot(data = m_predY, aes(x = curDur, y = m_m_mu_r - curDur,color=WMSize, shape = as.factor('Observation'))) +
  ...
  facet_grid(cols = vars(Exp)) +
  labs(x=" ", y="Reproduced bias (s)", shape=" ", color = "Memory Load")+theme_new+
...
```{r}
#log(IP)=[kr*M - (1-w_p)ks*M+sig2_post/2+ w_p*mu_p]/w_p
#sig2_post = (sigma_pr2 *sig_sm2)/(sigma_pr2 + sig_sm2);
...
write_csv(Bayparlist_spss %>% filter(Exp =='Exp4'), paste0(modelPath, '/rlt/Bayparlist_spss_exp4.csv'))
```
...
cohensD(sig_s2 ~ Exp,
        ...
        method = "paired")
cohensD(sig_s2 ~ Exp,
        data   = Bayparlist%>%filter(Exp %in% c('Exp3', 'Exp4')),
        method = "paired")
cohensD(sig_s2 ~ Exp,
        data   = Bayparlist%>%filter(Exp %in% c('Exp2', 'Exp4')),
        method = "paired")
```
...
#ggsave(paste0(getwd(), "/", modelPath, "figures/plt_rErrorScatter.png"), plt_rErrorScatter, width = 7, height = 4)
