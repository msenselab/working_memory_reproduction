...
real mu_lognormal(real mu, real variance){
  return(log(mu) - 0.5*log(1+variance/mu*mu));
}

real variance_lognormal(real mu, real variance){
  return(log(1+variance/mu*mu));
}

// self-defined function for predication
matrix predictor_rng(real[] x, int[] size, real[] gap, real ks, real ls, real ts, real kr, real sig_s2, real sigma_pr2,  real mu_pr_log, real sig_mn2, real[] par) {
...
real Gap[n];             //gap between production phase and reproduction phase in duration task
real Gap_new[846];       //new gap information between production phase and reproduction phase
...
real<lower=-1, upper = 1.3> mu_pr_log;  // mean of internal prior in log scale
real<lower=0, upper = 1> sig_pr2_log;  // sigma^2 of prior in log scale
real<lower=0, upper = 1> sig_mn2; //square of sigma of motor noise caused by WM task
} 
