...
```{r}
AllExpData = read.csv(paste0("../data/AllValidData.csv")) %>% filter(valid == 1)
AllExpData[which(AllExpData$Exp == "Exp4"),"Exp"] = "exp1"
AllExpData[which(AllExpData$Exp == "Exp5"),"Exp"] = "exp2"
AllExpData[which(AllExpData$Exp == "Exp2"),"Exp"] = "exp3"
AllExpData[which(AllExpData$Exp == "Exp3"),"Exp"] = "exp4"
AllExpData[which(AllExpData$Exp == "Exp1"),"Exp"] = "exp5"
AllExpData$WMFct <- factor(AllExpData$WMSize, labels = c("low","medium","high"))
AllExpData$Exp = as.factor(AllExpData$Exp)
AllExpData$NSub = as.numeric(AllExpData$Exp)*100 + AllExpData$NSub
AllExpData$bias = AllExpData$repDur - AllExpData$curDur
DataExp1to5 <- AllExpData[order(AllExpData$Exp),]
saveRDS(DataExp1to5, file = "../data/ValidData_Exp1to5.rds")

Data_Exp6 <- read.csv(paste0("../data/Exp6.csv")) %>% filter(valid == 1)
Data_Exp6$WMFct <- factor(Data_Exp6$WMSize, labels = c("low","medium","high"))
Data_Exp6$Exp = as.factor(Data_Exp6$Exp)
Data_Exp6$bias = Data_Exp6$repDur - Data_Exp6$curDur
saveRDS(Data_Exp6, file = "../data/ValidData_Exp6.rds")
```
...
m_Exp1 = Data_Exp1 %>% group_by(NSub, curDur, WMSize, WM2) %>% summarise(mbias = mean(bias)*1000)
#Data_Exp1$WM = as.factor(Data_Exp1$WMSize)
#Data_Exp1$WMSize = (Data_Exp1$WMSize -1)/2
#Data_Exp1$WM2 = Data_Exp1$WMSize^2
#Data_Exp1$curDur = Data_Exp1$curDur - 1.1

# map WMSize to numeric
m_Exp1$WMSize = as.factor(m_Exp1$WMSize)
m_Exp1$curDur = as.factor(m_Exp1$curDur)
...
```{r}
library(ez)
ezANOVA(m_Exp1, dv = mbias, wid = NSub, within = .(curDur, WMSize), type = 3)
...
```{r}
# linear mixed-effect model with dependent variable bias, fixed effect duration, WMSize, and their interaction, random effect NSub
m_Exp2 = Data_Exp2 %>% group_by(NSub, curDur, WMSize, gap) %>% summarise(mbias = mean(bias)*1000)
# map WMSize to numeric 
#m_Exp2$WMSize = as.numeric(m_Exp2$WMSize)
model_2 = lmer(mbias ~ curDur * WMSize*gap + (1|NSub), data = m_Exp2)
# summary of the model
summary(model_2)
```
...
```
```{r}
# caclulate average bias for each WMSize from mRpr_Exp1_sub
mBias_Exp1 = mRpr_Exp1_sub %>% group_by(WMSize) %>% 
  summarize(m_bias = mean(mBias)*1000, seBias = sd(mBias)/sqrt(n())*1000)
mBias_Exp1
mSlope_Exp1 = tb_SlpIP_Exp1_sub %>% group_by(WMSize) %>% 
  summarize(m_slope = mean(slope), se_slope = sd(slope)/sqrt(n()))
mSlope_Exp1
# left join mBias and mSlope
mBiasSlope_Exp1 = left_join(mBias_Exp1, mSlope_Exp1, by = "WMSize")
# factor WMSize as low, medium, and high
mBiasSlope_Exp1$WMSize <- factor(mBiasSlope_Exp1$WMSize, levels = c(1,3,5), labels=c("low", "medium", "high"))
```
```{r}
# plot bias and slope as 2D dot with SE, separate for each WMSize
plt_biasSlope_Exp1 <- ggplot(mBiasSlope_Exp1, aes(x=m_bias, y=m_slope, color = WMSize)) +
  geom_point(size=3) +
  geom_text(aes(label=WMSize), hjust=-0.2, vjust=-0.2) +
  geom_errorbar(aes(ymin=m_slope-se_slope, ymax=m_slope+se_slope), width = 0 ) +
  geom_errorbarh(aes(xmin=m_bias-seBias, xmax=m_bias+seBias)) +
  labs(x="Reproduction bias (ms)", y="Reproduction bias slope") +
  theme_new + colorSet3 + theme(legend.position = "none")
plt_biasSlope_Exp1

# save figure
ggsave(paste0(getwd(), "/figures/plt_biasSlope_Exp1.png"), plt_biasSlope_Exp1, width = 5, height = 5)
