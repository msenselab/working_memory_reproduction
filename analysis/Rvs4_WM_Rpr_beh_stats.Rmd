---
title: "Analyses for MS_Rev of WM_Rpr"
author: "Jiao Wu"
date: "2024-05-23"
...
Due to a failure of replication for Exp3, we revise the whole structure of the manuscript as follows:
Exp 1 <-- original Exp 4
Exp 2 <-- original Exp 5
Exp 3 <-- original Exp 2
Exp 4 <-- original Exp 3
Exp 5 <-- original Exp 1
Exp 6a,b,c <-- original Exp 6a,b,c

The story is that, first, we aim to explore the effect of WM on temporal reproduction (Exp 1). Then  (in Exp 2) we explore if the encoding-reproduction gap has an effect on temporal reproduction, and if yes, whether there is an interaction between WM effect and the gap effect.
Next, we systematically explore the WM effect on each phase (encoding, reproduction, and none for Exp 3, 4, 5).
...
To shorten the whole manuscript, we use a linear fitting on the Reproduction as a function of Duration, and report the statistics of the estimated slope (CT) and intercept (PSS).
...
plt_bar_WMCrr_Exp2 <- ggplot(WMCrrForExp2,aes(x=gap, y=mean_WMCrr, 
  ...
  theme_new + fillSet3 + theme(plot.title = element_text(hjust = 0.5))
plt_bar_WMCrr_Exp2
...
plt_bar_SlpIP_Exp1 <- ggplot(tb_SlpIP_Exp1, aes(x=WMSize,y=m_slope)) +
  ...
  theme_new + theme(legend.position = "top", legend.justification = c("center", "top"),
                    plot.title = element_text(hjust = 0.5))
plt_bar_SlpIP_Exp1
...
plt_bar_SlpIP_Exp2 <- ggplot(tb_SlpIP_Exp2, aes(x=gap,y=m_slope,
  ...
  geom_errorbar(aes(x=gap, ymin=m_slope-se_slope, ymax=m_slope+se_slope), 
                width=.1,  position = position_dodge(.9)) +
  geom_signif(annotations=c("n.s.","n.s.","n.s.","*","n.s.","**"), 
              tip_length = -0.1, vjust=2.5,
              y_position = c(-0.55, -0.58, -0.7, -0.6, -0.63, -0.75), 
              xmin = c(0.7, 1.05, 0.7, 1.7, 2.05, 1.7), 
              xmax = c(0.95, 1.3, 1.3, 1.95, 2.3, 2.3),
              color = 'black') +
...
plt_bar_SlpIP_Exp3 <- ggplot(tb_SlpIP_Exp3, aes(x=WMSize,y=m_slope)) +
  ...
  geom_errorbar(aes(ymin=m_slope-se_slope, ymax=m_slope+se_slope), 
                width=.1,  position = position_dodge()) +
  geom_signif(annotations=c("***","n.s."), tip_length = -0.1, vjust=2.5,
              y_position = c(-0.73,-0.78), xmin = c(1, 2.1), xmax = c(1.9, 3),
              color = 'black') +
  coord_cartesian(ylim = c(-0.85, 0)) +
  labs(x="Memory Load", y="Estimated slope", title="Reproduction bias slope") + 
  theme_new + theme(legend.position = "top", legend.justification = c("center", "top"),
...
plt_bar_SlpIP_Exp4 <- ggplot(tb_SlpIP_Exp4, aes(x=WMSize,y=m_slope)) +
  ...
  geom_errorbar(aes(ymin=m_slope-se_slope, ymax=m_slope+se_slope), 
                width=.1,  position = position_dodge()) +
  geom_signif(annotations=c("n.s.","n.s."), tip_length = -0.1, vjust=2.5,
              y_position = c(-0.65,-0.65), xmin = c(1, 2.1), xmax = c(1.9, 3),
              color = 'black') +
...
plt_bar_SlpIP_Exp5 <- ggplot(tb_SlpIP_Exp5, aes(x=WMSize,y=m_slope)) +
  ...
  geom_errorbar(aes(ymin=m_slope-se_slope, ymax=m_slope+se_slope), 
                width=.1,  position = position_dodge()) +
  geom_signif(annotations=c("n.s.","n.s."), tip_length = -0.1, vjust=2.5,
              y_position = c(-0.58,-0.58), xmin = c(1, 2.1), xmax = c(1.9, 3),
              color = 'black') +
...
plt_bar_SlpIP_Exp6 <- ggplot(tb_SlpIP_Exp6, aes(x=exp,y=m_slope,
  ...
  geom_errorbar(aes(ymin=m_slope-se_slope, ymax=m_slope+se_slope), 
                width=.1,  position = position_dodge(.9)) +
  geom_signif(annotations=c("**","n.s.","***","n.s.","n.s.","n.s.","n.s."),
              tip_length = -0.1, vjust=2.5,
              y_position = c(-0.48, -0.52, -0.62, -0.44, -0.44, -0.4, -0.4),
              xmin = c(0.7, 1.05, 0.7, 1.7, 2.05, 2.7, 3.05),
              xmax = c(0.95, 1.3, 1.3, 1.95, 2.3, 2.95, 3.3),
              color = 'black') +
  coord_cartesian(ylim = c(-0.7, 0)) +
...
```{r}
# plot data with estimated slope and IP
ggplot() +
  geom_hline(yintercept = 0, linetype='dashed') +
  geom_errorbar(data = mRpr, aes(curDur, m_bias, ymin = m_bias - se_bias, 
                               ymax = m_bias + se_bias, group = WMSize, color=as.factor(WMSize)), 
                width=.1, position = position_dodge(width = 0.02)) +
  geom_line(data = mRpr, aes(curDur, m_bias, group = WMSize, color=as.factor(WMSize)), 
             linewidth = 0.5, linetype = 5, position=position_dodge(width=0.02)) +
  geom_point(data = mRpr, aes(curDur, m_bias, group = WMSize, color=as.factor(WMSize)), 
             size = 1, stat="identity", position=position_dodge(width=0.02)) +
  scale_x_continuous(breaks=c(0.5, 0.8, 1.1, 1.4, 1.7)) +
  labs(x="Durations (s)", y="Reproduction bias (s)", 
       linetype ="data", color = "Memory Load")+
  geom_smooth(data = Data_Exp1, aes(x=curDur, y=bias, color=WMSize), method=lm, se=FALSE) +
  theme_new + colorSet3
