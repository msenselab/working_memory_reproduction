...
source('mytheme.R')
# model version
modelversion = 'rstan_WMSize_unconstrained'
...
AllDat_predY$gap = 1
AllDat_predY[which(AllDat_predY$Gap == 2.5),"gap"] = 2
...
```{r}
dat_Exp5 = dplyr::group_by(AllExpData%>%filter(Exp =='Exp. 5'), WMSize, NSub, gap) %>% 
...
dat_Exp5$gap<- factor(dat_Exp5$gap, labels = c("short", "long")) 
```

```{r}
plt_WMCrr_Exp5 = ggplot(data = dat_Exp5, 
                         aes(WMSize, mean_WMCrr, ymin = mean_WMCrr - se_WMCrr, ymax = mean_WMCrr + se_WMCrr, 
                             group = gap, color = gap)) + 
  ...
  labs(
    ...
    shape = "Gap",
    linetype = "Gap",
    y = "Mean accuracy in WM task in Exp. 5"
  ) +
  theme_new
...
```{r}
model <-ezANOVA(data= AllExpData%>%filter(Exp =='Exp. 5')%>%dplyr::group_by(NSub, WMSize, gap)%>%dplyr::summarise(m_WMCrr = mean(WMCrr)), dv = m_WMCrr, wid = NSub, within = .(WMSize, gap), type = 3, detailed = T)
model
```
```{r}
...
```{r}
sub_WMCrr_Exp5 = AllExpData%>%filter(Exp =='Exp. 5')%>%dplyr::group_by(NSub, WMSize, gap)%>%dplyr::summarise(mWMCrr = mean(WMCrr))%>%pivot_wider(names_from = c("WMSize", "gap"), values_from = c(mWMCrr), names_sep="_")
write.csv(sub_WMCrr_Exp5,paste0(modelPath, '/rlt/sub_WMCrr_Exp5.csv'))
```


```{r}
Exp.labs.1line <- c("Exp. 1 Control", "Exp. 2 Encoding", "Exp. 3 Reproduction", "Exp. 4 Both phases", "Exp. 4 Both phases\n \ \ \ \ \ \ \ \ \ \ \ \ \ with a gap")
...
plt_WMCrr_2<-ggarrange(plt_WMCrr, plt_WMCrr_Exp5, common.legend = FALSE, ncol=2, nrow=1, widths = c(5.5,4), labels = c("a", "b"))
plt_WMCrr_2
...
mpredY_sub <- dplyr::group_by(AllDat_predY, curDur, Exp, NSub, WMSize) %>%
  dplyr::summarize(n = n(), 
                   ...
                   predcv_rerr = mean(abs(pred_cv-cv)/cv), .groups = "drop")
```
...
write_csv(mpredY_sub_new%>%filter(Exp == 'Exp. 4'), paste0(modelPath, '/rlt/RP_Bias_exp4.csv'))
write_csv(mpredY_sub_new%>%filter(Exp == 'Exp. 5'), paste0(modelPath, '/rlt/RP_Bias_Exp5.csv'))
...
write_csv(mpredY_sub_WMsize%>%filter(Exp == 'Exp. 4'), paste0(modelPath, '/rlt/RP_Bias_WMsize_exp4.csv'))
write_csv(mpredY_sub_WMsize%>%filter(Exp == 'Exp. 5'), paste0(modelPath, '/rlt/RP_Bias_WMsize_Exp5.csv'))
...
AllDat_newY$gap = 1
AllDat_newY[which(AllDat_newY$Gap == 2.5, AllDat_newY$Exp == "Exp. 5"), "gap"] = 2
...
```{r}
AllDat_predY$Exp <- factor(AllDat_predY$Exp,
                           levels = c("Exp. 2", "Exp. 3", "Exp. 1", "Exp. 4", "Exp. 5"))

AllDat_newY$Exp <- factor(AllDat_newY$Exp,
                          levels = c("Exp. 2", "Exp. 3", "Exp. 1", "Exp. 4", "Exp. 5"))
Exp.labs.2lines <- c("Exp. 3\n Baseline", "Exp. 1\n Encoding", "Exp. 2\n Reproduction", "Exp. 4\n Both", "Exp. 5\n Both(with gap)")
names(Exp.labs.2lines) <- c("Exp. 1", "Exp. 2", "Exp. 3", "Exp. 4", "Exp. 5")
RP_bias  <- ggplot(data = m_predY%>%filter(Exp !="Exp. 5"), aes(x = curDur, y = m_m_repDur - curDur, color=as.factor(WMSize), shape = as.factor(WMSize))) +
  geom_point(size=2, alpha = 0.5)+
  geom_line(data= m_newY%>%filter(Exp !="Exp. 5"), aes(x=curDur, y=m_mu_r-curDur, color=WMSize)) +
...
```{r}
RP_bias_Exp5  <- ggplot(data = AllDat_predY %>% filter(Exp == 'Exp. 5')%>% 
...
```{r}
## Figures in the MS
RP_bias_all<-ggarrange(RP_bias, RP_bias_Exp5, common.legend = FALSE, ncol=2, nrow=1, widths = c(4,1.4),  labels = c("a", "b"))
...
RP_bias_1_5_bw <- ggplot(data = AllDat_predY %>% 
                         dplyr::group_by(Exp, curDur, WMSize, gap) %>% 
                         dplyr::summarize(m_mu_r = mean(mu_r), m_repDur = mean(repDur), n= n(), se_mu_r = sd(mu_r)/sqrt(n-1)), 
                      aes(x = curDur, y = m_repDur - curDur, 
  ...
  scale_shape_manual(values = c(16, 1, 17, 2, 15, 0)) +   # 根据 WMSize 数量自动区分
  scale_linetype_manual(values = c("solid", "dotted")) +   # gap 的区分（如需更多可调整）
  # ---------------------
...
ggsave(paste0(getwd(), "/", modelPath, "/figures/RP_bias_1_5_bw.png"), 
       RP_bias_1_5_bw, width = 9, height = 4)
...
```{r}
AllExpData[which(AllExpData$Exp != "Exp. 5"), "gap"] = 1
...
```{r}
RP_bias_obs  <- ggplot(data = m_obsRP%>%filter(Exp != 'Exp. 5'), aes(x = curDur, y = m_m_repDur-curDur,shape = as.factor(WMSize), color=as.factor(WMSize))) +
  geom_point()+
  geom_line()+
...
```
```{r}
RP_bias_obs_5  <- ggplot(data = m_obsRP%>%filter(Exp == 'Exp. 5'), aes(x = curDur, y = m_m_repDur-curDur,shape = as.factor(WMSize), color=as.factor(WMSize), linetype = factor(gap))) +
  geom_point()+
  geom_line()+
  geom_errorbar(width=.1,  aes(ymin = m_m_repDur-curDur - m_se_repDur, ymax = m_m_repDur -curDur + m_se_repDur, linetype = gap)) +
  geom_hline(yintercept = 0, linetype='dashed')+
...
```{r}
## Figures in the MS
RP_bias_obs_all<-ggarrange(RP_bias_obs, RP_bias_obs_5, common.legend = FALSE, ncol=2, nrow=1, widths = c(4,1.4),  labels = c("a", "b"))
...
wp_sub = m_wp %>%filter(Exp =='Exp. 5')%>% pivot_wider(names_from = c("WMSize", "gap"), values_from = c(m_wp), names_sep="_")
write.csv(wp_sub, paste0(modelPath, '/rlt/wp_sub.csv'))
```


```{r}
model <-ezANOVA(data= m_wp %>%filter(Exp =='Exp. 5'), dv = m_wp, wid = NSub, within = .(WMSize, gap), type = 3, detailed = T)
model
```
...
```{r}
#Observed Indifference Point for Exp. 5
...
```{r}
ezANOVA(data = obs_Inp_list%>%filter(Exp =='Exp. 5'), dv= inP, wid=NSub, within= .(gap, WMSize) )
```
...
```{r}
#Predicated Indifference Point for Exp.5
...
# plot predicated indifference points and slope of predicated RP
...
# linear_model = 'gap_linear_rstan_color'
...
obs_Inp_slope_Exp4_jasp <- obs_Inp_list_no_gap %>% filter(Exp =='Exp. 4') %>% dplyr::select(c("WMSize", "NSub", "inP","slope"))%>%
  pivot_wider(names_from = c("WMSize"), values_from = c(inP, slope), names_sep="_")
write.csv(obs_Inp_slope_Exp4_jasp, paste0(modelPath, '/rlt/obs_Inp_slope_Exp4_jasp.csv'))

obs_Inp_list_Exp5_jasp <- obs_Inp_list %>% filter(Exp == 'Exp. 5') %>% dplyr::select(c("WMSize", "NSub", "gap","inP", "slope"))%>% pivot_wider(names_from = c("WMSize", "gap"), values_from = c(inP,slope), names_sep="_")

write.csv(obs_Inp_list_Exp5_jasp, paste0(modelPath, '/rlt/obs_Inp_slope_Exp5_jasp.csv'))
```
...
pred_Inp_slope_Exp4_jasp <- pred_Inp_slope_no_gap %>% filter(Exp =='Exp. 4') %>% dplyr::select(c("WMSize", "NSub", "pred_inP","pred_slope"))%>%
  pivot_wider(names_from = c("WMSize"), values_from = c(pred_inP, pred_slope), names_sep="_")
write.csv(pred_Inp_slope_Exp4_jasp, paste0(modelPath, '/rlt/pred_Inp_slope_Exp4_jasp.csv'))

pred_Inp_slope_Exp5_jasp <- pred_Inp_list %>% filter(Exp == 'Exp. 5') %>% dplyr::select(c("WMSize", "NSub", "gap","pred_inP", "pred_slope"))%>% pivot_wider(names_from = c("WMSize", "gap"), values_from = c(pred_inP,pred_slope), names_sep="_")

write.csv(pred_Inp_slope_Exp5_jasp, paste0(modelPath, '/rlt/pred_Inp_slope_Exp5_jasp.csv'))
```
